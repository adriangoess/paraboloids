...
def check_function_violation(function_string, quads, lins, conss, domain, dim=1, print_result=True, approx_below=True):
    ...
    assert function_string in ["sin", "cos", "exp", "x^3"], (f"only sin, cos, x^3 and exp implemented for function "
                                                             f"to approximate")
...
def check_approximation_violation(function_string, quads, lins, conss, domain, eps, dim=1, print_result=True,
    ...
    This is conducted by computing the minimum of (y - f + eps) where y >= p_i for each i and stop if this value is
    negative.
    ...
    for name, coefs in zip(names, [quads, lins, conss]):
        ...
        for entry in coefs:
            ...
            for sub_entry in entry:
                assert isinstance(sub_entry, (float, )), f"{name} entries for one paraboloid must be floats, but {sub_entry} is type {type(sub_entry)}"
