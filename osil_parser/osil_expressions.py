...
class OSILPower(object):
    ...
    def compute_bounds(self, variables):
        ...
        if lb_exponent == ub_exponent == int(ub_exponent):
            # distinguish positive exponents
            if ub_exponent >= 0:
                ...
                # for non-negative or non-positive bounds, we can compute bounds
                if lb_base >= 0 and ub_base >= 0:
                    if lb_base == 0:
                        self.upper_bound = np.inf
                    else:
                        self.upper_bound = lb_base ** ub_exponent
                    if ub_base is None or np.isinf(ub_base):
                        self.lower_bound = 0
                    else:
                        self.lower_bound = ub_base ** ub_exponent
                if lb_base <= 0 and ub_base <= 0:
                    # distinguish even and odd degrees, as they differ on the negative axis
                    if int(ub_exponent) % 2 == 0:
                        # for even degree, an upper bound of the base returns an upper bound of the expression
                        if ub_base == 0:
                            self.upper_bound = np.inf
                        else:
                            self.upper_bound = ub_base ** ub_exponent
                        if lb_base is None or np.isinf(-lb_base):
                            self.lower_bound = 0
                        else:
                            self.lower_bound = lb_base ** ub_exponent
                    else:
                        # for odd degree, upper bounds for the base return lower bounds for the expression
                        if ub_base == 0:
        ...
        # for 0 bases and negative exponents, apply nothing
        elif (lb_base == 0 or ub_base == 0) and ub_exponent < 0:
            self.lower_bound = None
            self.upper_bound = None
        else:
        ...
        if self.lower_bound is None or np.isinf(-self.lower_bound):
            self.lower_bound = None
        if self.upper_bound is None or np.isinf(self.upper_bound):
            self.upper_bound = None
